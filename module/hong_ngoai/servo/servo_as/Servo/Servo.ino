  ...
  if (IrReceiver.decode()) {
    ...
    if (address == MY_ADDRESS || isRepeat) {
      ...
      if (!isRepeat && address == MY_ADDRESS) {
        ...
        // CHUYỂN MODE SÁNG
        // =====================
        if (command == CMD_BRIGHT) {
          // Reset trạng thái nếu đang từ Dark chuyển sang
          if (isDarkMode == true) {
            servoHigh = false;
            myServo.write(0);
            lastMoveTime = millis();
        ...
        // CHUYỂN MODE TỐI
        // =====================
        else if (command == CMD_DARK) {
          // Reset trạng thái nếu đang từ Bright chuyển sang
          if (isDarkMode == false) {
            servoHigh = false;
            myServo.write(0);
            lastMoveTime = millis();
...
if (!isStopped && (millis() - lastReceiveTime > TIMEOUT)) {
  ...
  if (isDarkMode) {
    if (millis() - lastMoveTime >= 700) {
      ...
      if (servoHigh) {
        myServo.write(180);
        Serial.println("DARK -> 180");
      } else {
        myServo.write(0);
        Serial.println("DARK -> 0");
      }
    }
  }
